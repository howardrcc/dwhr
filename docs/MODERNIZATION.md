...
- **W9 — Highcharts → permissive-license charting backend.** Replace `highcharter` with `echarts4r` (Apache-2.0) or `billboarder` (MIT). Removes the commercial-license footgun for downstream `dwhr` deployers. Out of scope for the CRAN-prep modernization (4-6 week effort competing with W3/W6 budget); a v3.0-class change. Full analysis and feature-by-feature comparison in [`docs/CHARTING-ALTERNATIVES.md`](CHARTING-ALTERNATIVES.md). Acceptance criteria when scoped: 15 example apps render equivalently, `presType = 'highCharts'` smoke test passes against the new backend, no new commercial-license dependency in `Imports`/`Suggests`.
