...
| 2026-04-26 | **Highcharts license analysis captured; migration deferred to W9.** Highcharts JS is dual-licensed CC BY-NC 3.0 / commercial; "non-commercial" excludes internal corporate dashboards, government, and (per Highsoft) hospitals. `highcharter` redistributes the JS bundle with its own notice; `dwhr` does not redistribute, but downstream commercial deployers inherit a Highcharts commercial-license obligation. **Action this phase**: ship a commercial-license notice in `README.md` (new "Notice — Highcharts" section) and `DESCRIPTION` (one sentence appended to `Description:`), mirroring `highcharter`'s pattern. **Deferred**: switching the charting backend. Full analysis with feature-by-feature comparison of `plotly`, `echarts4r`, `billboarder`, and `r2d3` (D3) lives in [`docs/CHARTING-ALTERNATIVES.md`](CHARTING-ALTERNATIVES.md). **Two co-equal candidates** if/when W9 is scoped, depending on priority: (a) **`echarts4r`** (Apache-2.0) for the fastest correct port — feature surface maps almost 1:1, ~4-6 weeks; (b) **`r2d3`** (D3 BSD-3 + r2d3 MIT) for the lowest long-term lock-in — eliminates the chart-library middle layer, fits the existing `inst/www/starExtend.js` imperative-bridge pattern, ~7-11 weeks but no upstream rug-pull risk. `billboarder` is a third option (lighter wrapper, weaker on Highstock features). | The migration is a v3.0-class change competing for budget with W3 (DBI/odbc) and W6 (CI). Shipping the notice now fixes the consumer-blindsiding problem without expanding the modernization scope. The recorded analysis means W9 starts from a finished comparison; the two-candidate posture reflects that the right pick depends on whether dwhr is fundamentally a "framework with renderable charts" (→ r2d3) or "pre-built dashboards with chart library polish" (→ echarts4r) — a question for the maintainer to settle when W9 is scoped, not now. |
...
- **W9 — Highcharts → permissive-license charting backend.** Replace `highcharter` with one of: **`echarts4r`** (Apache-2.0; fastest port, ~4-6 weeks); **`r2d3`** + D3 (BSD-3 + MIT; lowest lock-in, ~7-11 weeks, fits the existing `starExtend.js` imperative-bridge pattern); or `billboarder` (MIT; lighter wrapper, ~5 weeks, weaker on Highstock features). Removes the commercial-license footgun for downstream `dwhr` deployers. Out of scope for the CRAN-prep modernization — a v3.0-class change competing with W3/W6 budget. Full analysis with per-candidate work breakdowns in [`docs/CHARTING-ALTERNATIVES.md`](CHARTING-ALTERNATIVES.md). The candidate choice depends on whether `dwhr` is positioned as a framework with interchangeable renderers (→ r2d3) or as opinionated dashboards over a polished chart library (→ echarts4r) — settle this when W9 is scoped. Acceptance criteria when scoped: 15 example apps render equivalently, `presType = 'highCharts'` smoke test passes against the new backend, no new commercial-license dependency in `Imports`/`Suggests`.
