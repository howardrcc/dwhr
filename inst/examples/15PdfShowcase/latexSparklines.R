...
.plot_line <- function (df) {
    ...
    paste0("\\spark ",
           paste0(round(as.numeric(t(as.matrix(df))),2),
                  collapse = ' '),
           " /\n")
}
...
latexSparkline <- function(
    ...
    line2 <- .process_input(x2,y2, name='line')[,c(1,2)]
    
    
    if (na.rm && !is.null(line2) && is.data.frame(line2)) {
        line2 <- line2[complete.cases(line2),]
    }
    ...
    # Calculating ranges
    if (is.na(xlim[1])) {
        xlim[1] <- min(line[,1], line2[,1], spikes[,1], dots[,1], na.rm=TRUE)
    }
    if (is.na(ylim[1])) {
        ylim[1] <- min(line[,2], line2[,1], spikes[,2], dots[,2], na.rm=TRUE)
    }
    if (is.na(xlim[2])) {
        xlim[2] <- max(line[,1], line2[,1], spikes[,1], dots[,1], na.rm=TRUE)
    }
    if (is.na(ylim[2])) {
        ylim[2] <- max(line[,2], line2[,1], spikes[,2], dots[,2], na.rm=TRUE)
    }
    ...
    line2[,1] <- .range_normalize(line2[,1], xlim)
    line2[,2] <- .range_normalize(line2[,2], ylim)
    ...
    if (!is.null(line) && is.data.frame(line) && nrow(line)) {
        result <- paste0(result, "\\definecolor{sparklinecolor}{named}{",lineColor,"}")
    ...
    if (!is.null(line2) && is.data.frame(line2) && nrow(line2)) {
        result <- paste0(result, "\\definecolor{sparklinecolor}{named}{",lineColor2,"}")
        line2.splits <- c(1,which(!complete.cases(line2)),nrow(line2))
        sparks <- sapply(1:(length(line2.splits)-1), function (i) {
            start <- line2.splits[i]
            end <- line2.splits[i+1]
            .plot_line(line2[start:end,])
        })
        result <- paste0(result, paste0(sparks, collapse=""))
